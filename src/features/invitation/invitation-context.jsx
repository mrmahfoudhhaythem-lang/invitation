...
export function InvitationProvider({ children }) {
  ...
  const invitationUid = useMemo(() => {
    ...
    // Frontend-only mode: if URL only carries guest name, skip UID/API entirely.
    if (!uidFromUrl && hasGuestParam) {
      return null;
    }
  ...
  useEffect(() => {
    ...
    // Frontend-only guest links should not retain old UID from localStorage.
    if (hasGuestParam && !hasUidInPath && !hasUidParam) {
      clearWeddingUid();
    }

    if (hasUidInPath || hasGuestParam || hasUidParam) {
      // Clean URL without reload after extracting params.
      if (hasInvitationData() || hasGuestParam) {
